...
function getPathAPI (
  ...
  if(process.env.DOCKER_MODE === 'local') {
    return `http://${API_DEVELOPMENT_HOSTNAME}:${API_DEVELOPMENT_PORT}/${API_DEVELOPMENT_PATHNAME_PREFIX}/${route}`;
  }
  if(process.env.DOCKER_MODE === 'cypress-testing') {
      return `http://${DOCKER_BACKEND_SERVICE_NAME}:${API_DEVELOPMENT_PORT}/${API_DEVELOPMENT_PATHNAME_PREFIX}/${route}`;
  }
  throw new Error(`process.env.DOCKER_MODE accepts only 'local' or 'cypress-testing' values. Received ${process.env.DOCKER_MODE} instead.`);
}
